...
class Collection implements \ArrayAccess, \Countable, \IteratorAggregate
{
    ...
    {
        if (!$this->has($id)) {
            throw new \InvalidArgumentException(sprintf('Error in %s::%s(): Collection does not have %s element', self::class, __FUNCTION__, $id));
        }
     ...
     * @return  static
     */
    public function intersect(self $collection)
    {
        $static = new static();
        ...
        foreach ($commonIds as $commonId) {
            $static->add($this->entities[$commonId]);
        }
     ...
     *
     * @param   EntityInterface  $entity     Entity
     * @param   bool          $overwrite  Force writing the entity if it already exists
     *
     * @return  bool
     *
     * @throws  \InvalidArgumentException
