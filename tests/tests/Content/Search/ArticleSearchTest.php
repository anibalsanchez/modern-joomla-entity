...
class ArticleSearchTest extends \TestCaseDatabase
{
    ...
    public function activeUserAccessFilterIsApplied()
    {
        ...
        $filteredIds = array_map(
            fn ($categoryData) => (int) $categoryData['id'],
            ArticleSearch::instance(
                [
    ...
    public function notStateFilterIsApplied()
    {
        $statuses = array_unique(
            array_map(
                fn ($itemData) => (int) $itemData['state'],
                ArticleSearch::instance(
                    [
                        'filter.not_state' => 1,
                        'list.limit'   => 0,
    ...
    protected function getDataSet()
    {
        ...
        $phpUnitExtensionsDatabaseDataSetCsvDataSet->addTable('jos_content', JPATH_TESTS_PHPROBERTO.'/tests/Content/Search/Stubs/Database/content.csv');
