...
namespace Extly\Joomla\Entity\Tests\Categories\Search;
...
class CategorySearchTest extends \TestCaseDatabase
{
    ...
    public function extensionFilterIsApplied()
    {
        ...
        foreach ($results as $result) {
            $this->assertSame('com_content', $result['extension']);
        }
    }
    ...
    public function parentFilterIsApplied()
    {
        ...
        foreach ($categories as $category) {
            $this->assertSame(37, (int) $category['parent_id']);
        }
    }
    ...
    public function publishedFilterIsApplied()
    {
        $statuses = array_unique(
            array_map(
                fn ($categoryData) => (int) $categoryData['published'],
                CategorySearch::instance(
                    [
                        'filter.published' => 1,
                        'list.limit' => 0,
