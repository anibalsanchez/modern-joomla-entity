...
namespace Extly\Joomla\Entity\Tests\Tags\Search;
...
class TagSearchTest extends \TestCaseDatabase
{
    ...
    public function activeLanguageFilterIsApplied()
    {
        ...
        foreach ($tags as $tag) {
            $this->assertSame('en-GB', $tag['language']);
        }
    }
    ...
    public function levelFilterIsApplied()
    {
        ...
        foreach ($tags as $tag) {
            $this->assertSame(3, (int) $tag['level']);
        }
    }
    ...
    public function parentFilterIsApplied()
    {
        ...
        foreach ($tags as $tag) {
            $this->assertSame(6, (int) $tag['parent_id']);
        }
    }
    ...
    public function publishedFilterIsApplied()
    {
        $statuses = array_unique(
            array_map(
                fn ($tagData) => (int) $tagData['published'],
                TagSearch::instance(
                    [
                        'filter.published' => 1,
                        'list.limit' => 0,
